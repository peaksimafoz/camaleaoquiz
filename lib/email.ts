...
export async function sendResultEmail(o: ResultEmail): Promise<void> {
  const color = o.primaryColor || '#10b981'
  const subject =
    (o.subject && o.subject.trim()) ||
    `Seu resultado: ${o.resultName || o.quizName}`
  const attachments = o.ebookUrl
    ? [{ filename: 'E-book.pdf', path: o.ebookUrl }]
    : []
...
function renderHtml(o: ResultEmail, color: string): string {
  const greeting = o.name ? `Olá, ${esc(o.name)}!` : 'Olá!'
  const text = esc(o.resultText).replace(/\n/g, '<br>')
  const cta =
    o.ctaUrl && o.ctaLabel
      ...
      : ''
  const ebookNote = o.ebookUrl
    ? `<p style="font-size:14px;color:#475569;margin:24px 0 0;">📎 Seu material está anexado neste e-mail. Bons estudos!</p>`
    : ''
...
<html lang="pt-BR"><body style="margin:0;background:#f1f5f9;padding:24px;font-family:Arial,Helvetica,sans-serif;">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;padding:32px;">
    ...
    <h1 style="font-size:22px;color:#0f172a;margin:0 0 12px;">${esc(o.resultName || o.quizName)}</h1>
    <p style="font-size:15px;line-height:1.6;color:#334155;margin:0 0 24px;">${text}</p>
    ${cta}
    ${ebookNote}
    <hr style="border:none;border-top:1px solid #e2e8f0;margin:28px 0 16px;">
    <p style="font-size:12px;color:#94a3b8;margin:0;">Você recebeu este e-mail porque respondeu ao quiz "${esc(o.quizName)}".</p>
  </div>
</body></html>`
}
